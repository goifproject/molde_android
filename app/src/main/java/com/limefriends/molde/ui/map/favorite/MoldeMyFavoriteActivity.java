...
//        ArrayList<FavoriteEntity> moldeFavoriteEntities = new ArrayList<>();
//        moldeFavoriteEntities.add(new FavoriteEntity("lkj", "홍익대학교", 37.55267706, 126.92438746));
//        moldeFavoriteEntities.add(new FavoriteEntity("lkj", "홍익대학교", 37.55267706, 126.92438746));
//        moldeFavoriteEntities.add(new FavoriteEntity("lkj", "홍익대학교", 37.55267706, 126.92438746));
    ...
    private void loadFavorite(int perPage, int page) {
        ...
        Call<FavoriteResponseInfoEntityList> call = favoriteService.getMyFavorite("lkj", perPage, page);
        ...
        call.enqueue(new Callback<FavoriteResponseInfoEntityList>() {
            ...
            public void onResponse(Call<FavoriteResponseInfoEntityList> call, Response<FavoriteResponseInfoEntityList> response) {
                if (response.isSuccessful()) {
                    ...
                    // 4. 호출 후 데이터 정리
                    List<FavoriteEntity> entities = fromSchemaToLocalEntity(response.body().getData());
                    // 6. 데이터 추가
    ...
    private List<FavoriteEntity> fromSchemaToLocalEntity(List<FavoriteResponseInfoEntity> schemas) {
